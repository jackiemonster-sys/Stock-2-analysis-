...
st.caption("結合技術面 (均線/MACD)、深度籌碼面 (三大法人/融資融券/動能) 與基本面的智慧評估")
...
def fetch_finmind_chip_data(stock_id):
    ...
    try:
        ...
        if data.get("status") == 200 and data.get("data"):
            ...
            df_inst['buy_sell'] = (df_inst['buy'] - df_inst['sell']) / 1000 # 轉張數
            recent_inst = df_inst.groupby('name')['buy_sell'].sum().round(0).to_dict()
            result["inst"] = recent_inst
    except Exception:
        pass

    # 2. 抓取融資融券 (TaiwanStockMarginPurchaseShortSale)
    try:
        url = "https://api.finmindtrade.com/api/v4/data"
        param_margin = {
        ...
        if data.get("status") == 200 and data.get("data"):
            ...
            if not df_margin.empty:
                ...
                short_sell_change = recent_5d['ShortSaleBuy'].sum() - recent_5d['ShortSaleSell'].sum() # 賣出 - 買進(償還)
...
if df is None or len(df) < 60:
    ...
    if margin_info:
        ...
        if price_change > 0 and m_change < 0:
            chip_reasons.append("✨ **價漲資減**：主力鎖碼或籌碼籌集，散戶解套退場（籌碼健康）。")
            chip_score += 1
        elif price_change > 0 and m_change > 0:
            chip_reasons.append("⚠️ **價漲資增**：散戶追高意願強或主力開槓桿，需留意追高風險。")
        elif price_change < 0 and m_change > 0:
    ...
    # 7. 圖表繪製 (含 K線、成交量、融資融券餘額、MACD)
    st.markdown("---")
    st.subheader("📉 技術與籌碼 (資券變化) 圖表分析")
    ...
    fig = make_subplots(
        ...
        subplot_titles=('K線與均線 (MA5/20/60)', '成交量 (Vol)', '信用交易 (融資與融券餘額)', 'MACD 指標')
    )
    ...
    # 3. 融資融券走勢圖
    margin_df = chip_data.get("margin", {}).get("df_margin")
    if margin_df is not None and not margin_df.empty:
        # 對齊 K 線的時間軸
        margin_df = margin_df.set_index('date')
        margin_df_chart = margin_df.reindex(df_chart.index).ffill()
        
        fig.add_trace(go.Scatter(
            x=margin_df_chart.index, y=margin_df_chart['MarginPurchaseTodayBalance'],
            line=dict(color='firebrick', width=1.5), name='融資餘額(張)'
        ), row=3, col=1)
        fig.add_trace(go.Scatter(
            x=margin_df_chart.index, y=margin_df_chart['ShortSaleTodayBalance'],
            line=dict(color='royalblue', width=1.5), name='融券餘額(張)'
        ), row=3, col=1)
    else:
        # 若抓取失敗退回 OBV 能量潮
