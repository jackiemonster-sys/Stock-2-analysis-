...
def get_taiwan_stock_list():
    ...
    default_map = {
        ...
        "2408 南亞科 (上市)": {"stock_id": "2408", "stock_name": "南亞科", "ticker_symbol": "2408.TW", "market_type": "上市"},
...
def fetch_finmind_chip_data(stock_id):
    ...
    try:
        ...
        res = requests.get(url, params=param_inst, headers=HTTP_HEADERS, timeout=8)
        data = res.json()
        if data.get("status") == 200 and data.get("data"):
            ...
            if not df_inst.empty:
                ...
                # 抓取籌碼最新日期
                latest_chip_date = df_inst['date'].iloc[-1]
                result["chip_date"] = latest_chip_date
    except Exception:
        pass
    ...
    try:
        ...
        res = requests.get(url, params=param_margin, headers=HTTP_HEADERS, timeout=8)
        data = res.json()
        if data.get("status") == 200 and data.get("data"):
...
def load_stock_data(symbol):
    try:
        ...
        return df, info, trade_date
    except Exception:
        return None, None, None

with st.spinner("正在讀取最新交易與籌碼數據..."):
    df, info, trade_date = load_stock_data(ticker_symbol)
...
if df is None or len(df) < 30:
    ...
    # 3. 核心卡片與日期顯示
    st.markdown("---")
    chip_date_display = chip_data.get("chip_date", "同步中")
    st.subheader(f"📊 {stock_name} ({ticker_symbol}) 核心指標")
    st.markdown(f"🗓️ **交易日（行情）：`{trade_date}`** ｜ 🏛️ **籌碼結算日：`{chip_date_display}`**")
